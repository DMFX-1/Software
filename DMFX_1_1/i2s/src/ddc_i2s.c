...
extern Int16 ping_pong_i2sRxLeftBuf[];
extern Int16 ping_pong_i2sRxRightBuf[];
#endif // ENABLE_RECORD
...
extern Uint16 codec_input_buffer_out_error;
...
)
{
    ...
    if ((dataCallback != NULL) && (dmaStatus == PSP_DMA_TRANSFER_COMPLETE))
    {
        ...
        //LOG_printf(&trace, "Post RX DMA SEM_DmaRxLeftComplete");
    }
    else
    {
        LOG_printf(&trace, "TX DMA Failed\n");
    }
}
...
)
{
    ...
    if (codec_input_buffer_avail == FALSE) /* check input FIFO overflow */
    {
        codec_input_buffer_out_error++;
        LOG_printf(&trace, "ERROR: codec input FIFO OVERFLOW");
    }
    codec_input_buffer_avail = FALSE;
    RxIntCounter++;
    if ((dataCallback != NULL) && (dmaStatus == PSP_DMA_TRANSFER_COMPLETE))
    {
