...
#ifdef ENABLE_RECORD
// loopback input to output
//#define ENABLE_RECORD_LOOPBACK

//#ifdef ENABLE_RECORD_LOOPBACK
// setup record processing path for right channel
#define ENABLE_STEREO_RECORD
//#endif // ENABLE_RECORD_LOOPBACK
...
/* I2S Tx buffer size */
#define MAX_I2S_TXBUFF_SZ           (MAX_TXBUFF_SZ_I2SSAMPS)

#define CODEC_OUTPUT_BUFFER_SIZE    (2*MAX_I2S_TXBUFF_SZ) // 6*MAX_I2S_TXBUFF_SZ
...
// since we need datapack to be disabled on I2S tx, we need it disabled on I2S rx therefore
// we get 2 words per DMA transfer so the offset into DMA buffers has to be twice as big
#define RXBUFF_SZ_ADCSAMPS          (64u) //(RXBUFF_SZ_MSEC*16) /* size of codec Rx ping or pong buffer in ADC samples */
#define RXBUFF_SZ_I2SSAMPS          (RXBUFF_SZ_ADCSAMPS) /* size of codec Rx ping or pong buffer in I2S samples */

#else /* ADC: 48kHz, I2S: 48kHz */
// since we need datapack to be disabled on I2S tx, we need it disabled on I2S rx therefore
// we get 2 words per DMA transfer so the offset into DMA buffers has to be twice as big
#define RXBUFF_SZ_ADCSAMPS          (4*64u)  //(RXBUFF_SZ_MSEC*48) ///* size of codec Rx ping or pong buffer in ADC samples */
#define RXBUFF_SZ_I2SSAMPS          (RXBUFF_SZ_ADCSAMPS) /* size of codec Rx ping or pong buffer in I2S samples */

#endif

/* I2S Rx buffer size */
#define I2S_RXBUFF_SZ               (RXBUFF_SZ_I2SSAMPS) /* 2x for DMA (32-bit transfers) */
