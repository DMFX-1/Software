...
#else
  /** 30NOV12 - Pratap: Added as workaround for chkdsk errors **/
  if((pAtaDrive->FatType == ATA_FAT_TYPE_32) && (save_cluster == 2))
  {
  	pAtaFile->StartCluster = 0;
  }
  else
  {
  	pAtaFile->StartCluster = save_cluster;
  }
  /** 30NOV12 - Pratap: End of chkdsk workaround **/
#endif
