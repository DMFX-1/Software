 ...
 *
 *      Length of data that can be transferred on any end point in this test
 *      should be multiple of 4.
 *      Read/Write operations with data length other than multiple of 4 from
 *      host can result in unexpected behavior of the test
 *
 ...
 * ================
 * 24-Oct-2008 Created
 * 14-May-2009 Added DMA word swap work around for C5515 PG1.4
 * 06-Aug-2010 Updated
 * 23-Jul-2012 Updated documentation
 * 30-Mar-2013 Modified to support new USB host tool
...
#pragma DATA_SECTION(usbDataBuffer1, ".buffer3")
Uint16    usbDataBuffer1[CSL_USB_DATA_SIZE];
#pragma DATA_SECTION(usbDataBuffer2, ".buffer3")
...
Uint16    deviceQualDesc[] = {0x060A, 0x0200, 0x0000, 0x4000, 0x0001};
...
#endif
//pUsbContext           hUsbDev = &gUsbContext;
CSL_Status            status;
...
CSL_Status CSL_startTransferCallback(void    *vhUsbDev,
                                     void    *vpeps);
CSL_Status CSL_completeTransferCallback(void    *vhUsbDev,
                                        void    *vpeps);
...
interrupt void usb_isr(void)
{
	...
	if(hUsbDev->dwIntSourceH & CSL_USB_GBL_INT_RESET)
	{
		...
		{
			USB_confDmaRx(&hpdrx, CSL_USB_EP1_PACKET_SIZE_FS, usbDataBuffer1);
			USB_dmaRxStart(CSL_USB_EP1);
		}
		else if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP2)
		{
			USB_confDmaRx(&hpdrx, CSL_USB_EP2_PACKET_SIZE_FS, usbDataBuffer2);
			USB_dmaRxStart(CSL_USB_EP2);
		}
		else if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP3)
		{
			USB_confDmaRx(&hpdrx, CSL_USB_EP3_PACKET_SIZE_FS, usbDataBuffer3);
			USB_dmaRxStart(CSL_USB_EP3);
		}
		else if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP4)
		{
			USB_confDmaRx(&hpdrx, CSL_USB_EP4_PACKET_SIZE_FS, usbDataBuffer4);
			USB_dmaRxStart(CSL_USB_EP4);
		}
		else{}
	}
	...
	if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP1)
	{
		...
		if (USB_isValidDataInFifoOut(&hUsbDev->pEpStatus[getEndPoint(outEp1)]))
		{
			// get the EP1 handle
		...
		USB_confDmaRx(&hpdrx, usb_income_num_bytes_ep1, usbDataBuffer1);
		USB_dmaRxStart(CSL_USB_EP1);
	}
	else if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP2)
	{
		...
		if (USB_isValidDataInFifoOut(&hUsbDev->pEpStatus[getEndPoint(outEp2)]))
		{
			// get the EP2 handle
		...
		USB_confDmaRx(&hpdrx, usb_income_num_bytes_ep2, usbDataBuffer2);
		USB_dmaRxStart(CSL_USB_EP2);
	}
	else if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP3)
	{
		...
		if (USB_isValidDataInFifoOut(&hUsbDev->pEpStatus[getEndPoint(outEp3)]))
		{
			// get the EP3 handle
		...
		USB_confDmaRx(&hpdrx, usb_income_num_bytes_ep3, usbDataBuffer3);
		USB_dmaRxStart(CSL_USB_EP3);
	}
	else if(hUsbDev->dwIntSourceL & CSL_USB_RX_INT_EP4)
	{
		...
		if (USB_isValidDataInFifoOut(&hUsbDev->pEpStatus[getEndPoint(outEp4)]))
		{
			// get the EP4 handle
		...
		USB_confDmaRx(&hpdrx, usb_income_num_bytes_ep4, usbDataBuffer4);
		USB_dmaRxStart(CSL_USB_EP4);
	}
	else
	{}
	...
	if (queuePend1 & 0x0400)
	{
		...
		if(intEp == CSL_USB_RX_INT_EP1)
		{
		    ...
		    USB_dmaRxStop(CSL_USB_EP1, usbDataBuffer1,
		                  CSL_USB_EP1_PACKET_SIZE_FS, FALSE);
#else
		    USB_dmaRxStop(CSL_USB_EP1, usbDataBuffer1,
		                  CSL_USB_EP1_PACKET_SIZE_FS, TRUE);
#endif
			/* This is to initiate filling the Tx FIFO with data for next read operation */
			hUsbDev->dwIntSourceL |= CSL_USB_TX_INT_EP1;
		}
		else if(intEp == CSL_USB_RX_INT_EP2)
		{
#if (defined(CHIP_C5505_C5515) || defined(CHIP_C5504_C5514) || defined(CHIP_C5517))
		...
		else if(intEp == CSL_USB_RX_INT_EP3)
		{
#if (defined(CHIP_C5505_C5515) || defined(CHIP_C5504_C5514) || defined(CHIP_C5517))
		    USB_dmaRxStop(CSL_USB_EP3, usbDataBuffer3,
		                  CSL_USB_EP3_PACKET_SIZE_FS, FALSE);
#else
		    USB_dmaRxStop(CSL_USB_EP3, usbDataBuffer3,
		                  CSL_USB_EP3_PACKET_SIZE_FS, TRUE);
#endif
			/* This is to initiate filling the Tx FIFO with data for next read operation */
			hUsbDev->dwIntSourceL |= CSL_USB_TX_INT_EP3;
		}
		else if(intEp == CSL_USB_RX_INT_EP4)
		{
#if (defined(CHIP_C5505_C5515) || defined(CHIP_C5504_C5514) || defined(CHIP_C5517))
		    USB_dmaRxStop(CSL_USB_EP4, usbDataBuffer4,
		                  CSL_USB_EP4_PACKET_SIZE_FS, FALSE);
#else
		    USB_dmaRxStop(CSL_USB_EP4, usbDataBuffer4,
		                  CSL_USB_EP4_PACKET_SIZE_FS, TRUE);
#endif
			/* This is to initiate filling the Tx FIFO with data for next read operation */
	...
	else if (queuePend1&0x100)
	{
		...
		else if(intEp == CSL_USB_TX_INT_EP2)
		{
            USB_dmaTxStop(CSL_USB_EP2);
		}
		else if(intEp == CSL_USB_TX_INT_EP3)
		{
            USB_dmaTxStop(CSL_USB_EP3);
		}
		else if(intEp == CSL_USB_TX_INT_EP4)
		{
...
#if (defined(CHIP_C5505_C5515) || defined(CHIP_C5504_C5514) || defined(CHIP_C5517))
		USB_confDmaTx(CSL_USB_EP2, &hpdtx, usb_income_num_bytes_ep2, usbDataBuffer2, FALSE);
#else
		USB_confDmaTx(CSL_USB_EP2, &hpdtx, usb_income_num_bytes_ep2, usbDataBuffer2, TRUE);
#endif
...
#else
		USB_confDmaTx(CSL_USB_EP3, &hpdtx, usb_income_num_bytes_ep3, usbDataBuffer3, TRUE);
#endif
...
#if (defined(CHIP_C5505_C5515) || defined(CHIP_C5504_C5514) || defined(CHIP_C5517))
		USB_confDmaTx(CSL_USB_EP4, &hpdtx, usb_income_num_bytes_ep4, usbDataBuffer4, FALSE);
#else
		USB_confDmaTx(CSL_USB_EP4, &hpdtx, usb_income_num_bytes_ep4, usbDataBuffer4, TRUE);
#endif
		USB_dmaTxStart(CSL_USB_EP4);
	}
 ...
 *
 *  \param  vhUsbDev - USB context structure
                                     ...
                                     void    *vpeps)
{
	...
	hUsbDev  = (pUsbContext)vhUsbDev;
	peps      = (pUsbEpStatus)vpeps;

    if((hUsbDev == NULL) || (peps == NULL))
    {
	...
	if(pTransfer->dwFlags == CSL_USB_OUT_TRANSFER)
	{
		...
		{
			status = USB_handleRx(hUsbDev, getEndPoint(outEp1));
		}
		else if(peps->dwEndpoint == CSL_USB_EP2)
		{
			status = USB_handleRx(hUsbDev, getEndPoint(outEp2));
		}
		else if(peps->dwEndpoint == CSL_USB_EP3)
		{
			status = USB_handleRx(hUsbDev, getEndPoint(outEp3));
		}
		else
		{
			status = USB_handleRx(hUsbDev, getEndPoint(outEp4));
		}
	}
	else if(pTransfer->dwFlags == CSL_USB_IN_TRANSFER)
	{
		...
		{
			status = USB_handleTx(hUsbDev, getEndPoint(inEp1));
		}
		else if(peps->dwEndpoint == CSL_USB_EP2)
		{
			status = USB_handleTx(hUsbDev, getEndPoint(inEp2));
		}
		else if(peps->dwEndpoint == CSL_USB_EP3)
		{
			status = USB_handleTx(hUsbDev, getEndPoint(inEp3));
		}
		else
		{
			status = USB_handleTx(hUsbDev, getEndPoint(inEp4));
		}
	}
	else
	{
