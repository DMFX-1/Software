...
/** @file csl_emif_nand_polled_example.c
 *
 *  @brief EMIF functional layer polled mode example source file
 *
 *
 * \page    page13  CSL EMIF NAND EXAMPLE DOCUMENTATION
 *
 * \section EMIF1   EMIF NAND EXAMPLE1 - POLL MODE TEST
 *
 * \subsection EMIF1x    TEST DESCRIPTION:
 ...
 *
 * \subsection EMIF1y    TEST PROCEDURE:
 ...
 *  \return None
 */
void CSL_testQueryFucn(CSL_EmifHandle    hEmif,
                       Uint16            nandOptions);
...
//    status = ASYNC_config(hEmif, &asyncCfg);
	if(status != CSL_SOK)
	{
	...
	if(gNandType == 1)
	{
		...
		CSL_testQueryFucn(hEmif, cslNandIdLookup[gDevIndex].nandOptions);
	}
	...
	{
		status = NAND_setAddress(hEmif, flashAddress);
		if(status != CSL_SOK)
		{
	...
	/* Write Nand page */
	status = NAND_writeNBytes(hEmif, gNandPageSize, gnandWriteBuf, 0, 0);
	if(status != CSL_SOK)
	{
	...
	/* Read Nand page */
	status = NAND_readNBytes(hEmif, gNandPageSize, gnandReadBuf, 0, 0);
	if(status != CSL_SOK)
	{
