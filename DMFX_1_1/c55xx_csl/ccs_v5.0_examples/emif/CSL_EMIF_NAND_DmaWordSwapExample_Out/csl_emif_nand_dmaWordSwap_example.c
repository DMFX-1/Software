...
/** @file csl_emif_nand_dmaWordSwap_example.c
 *
 ...
 *
 * \section NAND4  EMIF NAND EXAMPLE4 - DMA WORD SWAP TEST
 *
 ...
 * Revision History
 * ================
 * 18-Feb-2013 Created for verifying DMA word swap work around on C5515 PG1.4
 * ============================================================================
 */
...
CSL_Status  CSL_nand_dmaWrite_pollRead(void)
{
	...
	CSL_NAND_CHANGE_ACCESSTYPE(CSL_EMIF_WORD_ACCESS);
	...
	CSL_NAND_CHANGE_ACCESSTYPE(CSL_EMIF_LOWBYTE_ACCESS);
	...
	/* Read Nand page */
	status = NAND_readNBytes(hEmif,gNandPageSize,gnandDmaReadBuf,0,0);
	if(status != CSL_SOK)
	{
...
CSL_Status  CSL_nand_pollWrite_dmaRead(void)
{
	...
	/* Write Nand page */
	status = NAND_writeNBytes(hEmif,gNandPageSize,gnandDmaWriteBuf,0,0);
	if(status != CSL_SOK)
	{
