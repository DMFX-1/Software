 ...
 *
 *
 * \page    page15  CSL RTC EXAMPLE DOCUMENTATION
 *
 ...
 * Revision History
 * ================
 * 15-May-2009 Created for verifying RTC compensation on C5515 PG1.4
...
void main()
{
   ...
   /////
	...
	printf("\nRTC positive compensation does not work for the values that ");
	printf("are multiples of 10 on chip C5505 and C5504.\n");
	printf("RTC_setCompensation() function will error during this test.\n");
	printf("This issue is not present on C5515 and C5514\n");

	printf("\n\nRTC Compensation Tests Complete!!\n");
	...
	if (CSL_SOK == status)
	{
		...
		while(!breakLoop)
		{
			...
			/* Break the program execution after a change in hours component and
			   ...
			   this period
			 */
			if((getTime.mins > initTime.mins) && (getTime.secs > 0))
			{
				breakLoop = TRUE;
			}
		}
