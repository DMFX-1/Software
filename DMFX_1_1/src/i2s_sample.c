...
Uint16 ping_i2sTxLeftBuf[MAX_I2S_TXBUFF_SZ+4];
#pragma DATA_ALIGN(ping_i2sTxRightBuf, 4);
Uint16 ping_i2sTxRightBuf[MAX_I2S_TXBUFF_SZ+4];
#pragma DATA_ALIGN(pong_i2sTxLeftBuf, 4);
Uint16 pong_i2sTxLeftBuf[MAX_I2S_TXBUFF_SZ+4];
#pragma DATA_ALIGN(pong_i2sTxRightBuf, 4);
Uint16 pong_i2sTxRightBuf[MAX_I2S_TXBUFF_SZ+4];

#else  // ENABLE_STEREO_PLAYBACK
#pragma DATA_ALIGN(i2sTxLeftBuf, 4);
Uint16 i2sTxLeftBuf[MAX_I2S_TXBUFF_SZ];
#pragma DATA_ALIGN(i2sTxRightBuf, 4);
Uint16 i2sTxRightBuf[MAX_I2S_TXBUFF_SZ];
...
Int16 ZeroBuf[MAX_I2S_TXBUFF_SZ];
#endif /* SAMPLE_BY_SAMPLE_PB */

//#ifdef ENABLE_RECORD
/* Codec input ping/pong buffer (Left ch.) */
#pragma DATA_ALIGN(ping_pong_i2sRxLeftBuf, 4);
Int16 ping_pong_i2sRxLeftBuf[2*I2S_RXBUFF_SZ]; /* 2x for ping/pong */
Int16 left_rx_buf_sel = 0;

/* Codec input ping/pong buffer (Right ch.) */
#pragma DATA_ALIGN(ping_pong_i2sRxRightBuf, 4);
Int16 ping_pong_i2sRxRightBuf[2*I2S_RXBUFF_SZ]; /* 2x for ping/pong */
...
Int16 ping_pong_i2sTxBuf[2*2*MAX_I2S_TXBUFF_SZ]; /* 2x for ping/pong */
...
Bool codec_input_buffer_avail;
Uint16 codec_input_buffer_out_error;
Int16  TxIntCounter = 0;
Int16  RxIntCounter = 0;

Uint16 i2sTxBuffSz;
Uint16 i2sTxBuf_idx;
int gHl, gCl, gHm, gCm, gHh, gCh;
int Gl, Gm, Gh;
...
)
{
    ...
    if (pI2sInitPrms->enablePlayback == TRUE)
    {
        ...
        if (pI2sInitPrms->sampleBySamplePb == FALSE)
        {
            ...
            dmaTxConfig.callback        = I2S_DmaTxLChCallBack; //I2S_DmaTxCallBack
            ...
            dmaTxConfig.callback  =  I2S_DmaTxRChCallBack; //I2S_DmaTxCallBack
    ...
    if (pI2sInitPrms->enableRecord == TRUE)
    {
        ...
        if (pI2sInitPrms->sampleBySampleRec == FALSE)
        {
            ...
            dmaRxConfig.callback = I2S_DmaRxRChCallBack; /* NOTE: currently stereo record not handled in this fxn */
...
#if 0
    STS_set(&HWI_INT8_STS, CLK_gethtime());
#endif
...
#if 0  //DCM remove USB from Record Audio
...
{ /* SampleBySample */
    ...
    //if (set_record_interface >= START_REC_WAIT_NUM_FRAMES)
#if 0  // Remove from USB Record Audio    
    if (h_usb_int_tcount > 1) /* wait for IN tokens from Host */
    {
        /* Get sample from I2S input, place in input circular buffer */
...
{ /* SampleBySample */
...
#if 0
    STS_delta(&HWI_INT8_STS, CLK_gethtime());
    STS_set(&HWI_INT8_STS, CLK_gethtime());
#endif
    ...
    *(ioport volatile unsigned *)I2S_I2STXRT1 = codec_output_buffer[codec_output_buf_samp_cnt++];
    ...
    if (codec_output_buf_samp_cnt >= i2sTxBuffSz)
    {
        ...
        {
            codec_output_buffer_out_error++;
            LOG_printf(&trace, "ERROR: codec output FIFO UNDERFLOW: %d", codec_output_buf_samp_cnt);
            //while(1);
        }
	#endif
        ...
        codec_output_buf_samp_cnt = 0;
        codec_output_buffer_avail = FALSE;
        SEM_post(&SEM_TxSampsSent);
        //LOG_printf(&trace, "Post SEM_TxSampsSent\n");
    }
#endif /* SAMPLE_BY_SAMPLE_PB */
}
...
{
    //memset(ping_pong_i2sTxBuf, 0, 2*2*MAX_I2S_TXBUFF_SZ);
    memset(ping_pong_i2sTxLeftBuf, 0, 2*MAX_I2S_TXBUFF_SZ);
    memset(ping_pong_i2sTxRightBuf, 0, 2*MAX_I2S_TXBUFF_SZ);
    codec_output_buffer = &ping_pong_i2sTxBuf[0];
    tx_buf_sel = 0x0;
    left_rx_buf_sel = 0x0;	// DCM 09/06/2013 There is an issue on how to synchronize initially DMA_Isr ping-pong buffer and RecInBuf writing
    codec_output_buf_samp_cnt = 0;
    codec_output_buffer_avail = TRUE;
    codec_output_buffer_out_error = 0;
    codec_input_buffer_avail = TRUE;
    codec_input_buffer_out_error = 0;
    TxIntCounter = 0;
    RxIntCounter = 0;
}
