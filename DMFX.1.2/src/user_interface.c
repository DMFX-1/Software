...
interrupt void PenIsr(void);
extern void VECSTART(void);
#endif

LcdMsgType UserInterfaceSwi(void);
...
)
{
...
#ifdef SAR_INT_MODE
    /* Initialize channel */
    status = SAR_chanInit(gsSarHandle);
    if(status != CSL_SOK)
    {
        *pCslStatus = status;
        return UI_CSL_FAIL;
    ...
    status = SAR_chanCycSet(gsSarHandle,CSL_SAR_CONTINUOUS_CONVERSION);
    if(status != CSL_SOK)
    {
        *pCslStatus = status;
        return UI_CSL_FAIL;
    ...
    status = SAR_A2DMeasParamSet(gsSarHandle, /*CSL_PEN_PRESS_DOWN,*/ CSL_KEYPAD_MEAS, &sarChanNo);
    if ((status != CSL_SOK) || (sarChanNo != CSL_SAR_CHAN_3 /*5*/))
    {
        *pCslStatus = status;
        return UI_CSL_FAIL;
    }
    ...
    /* Configure GPT1 */
    tmrPrd = (Uint32)((Float32)cpuFreq/2.0/pbnSampFreq/1048.0 + 0.5)-1; //Divided by 256.0 to to reduce bandwidth
...
void MenuDisplayTask()
{
    CSL_Status cslStatus;
    Int16 status;

    //DCM: Initialize User Interface independently of USB application
...
)
{
    ...
    switch (pbnState)
    {
        case UI_MIC_MUTE:
            //pHidReport[1] |= 0x1 << 2;
            LcdMsg = LCD_MSG_BUTTON_NONE;
            break;
        case UI_PB_VOL_UP:
            //pHidReport[0] |= 0x1 << (8+0);
            //Change_Volume(1, 0);
            //Change_Volume(1, 1);
            LcdMsg = LCD_MSG_BUTTON_R;
            //MBX_post(&MBX_lcdmsg, &LcdMsg, 1);
            break;
        case UI_PB_VOL_DN:
            //pHidReport[0] |= 0x1 << (8+1);
            //Change_Volume(-1, 0);
            //Change_Volume(-1, 1);
            LcdMsg = LCD_MSG_BUTTON_L;
            //MBX_post(&MBX_lcdmsg, &LcdMsg, 1);
            break;
        case UI_PB_MUTE:
            ...
            //Change_Volume(0, 0);
            //Change_Volume(0, 1);
            LcdMsg = LCD_MSG_BUTTON_BOTH;
            //MBX_post(&MBX_lcdmsg, &LcdMsg, SYS_FOREVER);
            break;
        case UI_PUSH_BUTTON_NONE:
            LcdMsg = LCD_MSG_BUTTON_NONE;
            break;
        default :
            LcdMsg = LCD_MSG_BUTTON_NONE;
...
void PenIsr(void)
{
	...
	LcdMsg = UserInterfaceSwi();
	if(LcdMsg != LCD_MSG_BUTTON_NONE)
	{
		menu(LcdMsg);
		SetDistParams();
	}		
...
LcdMsgType UserInterfaceSwi(void)
{
    ...
    {
        /* Generate HID report */
        LcdMsg = genHidReport(pbnState, gHidReport);
